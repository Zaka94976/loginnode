  ...
  async sendOTP(req, res, next) {
    try {
      ...
      const {
        identifier,
        identifierType
      } = validateSendOTP(req.body);
      const provider = otpProviderFactory.getProvider();
      if (!provider.validateIdentifier(identifier)) {
        throw new AppError(`Invalid ${identifierType || "identifier"} format`, 400, "INVALID_IDENTIFIER");
      }
      const cooldown = await redisService.getCooldown(identifier);
      if (cooldown > 0) {
        throw new AppError(`Please wait ${cooldown} seconds before requesting a new OTP`, 429, "RATE_LIMITED");
      }
      const otp = generateOTP(config.otp.digits);
      await Promise.all([redisService.storeOTP(identifier, otp, config.otp.expirySeconds), redisService.setCooldown(identifier, config.otp.resendCooldownSeconds)]);
  ...
  async verifyOTP(req, res, next) {
    try {
      ...
      if (otp !== storedOTP) {
        ...
        throw new AppError(`Invalid OTP. ${remainingAttempts} attempts remaining`, 400, "INVALID_OTP");
      }
      await Promise.all([redisService.deleteOTP(identifier), redisService.resetAttempts(identifier)]);
      const user = {
        identifier,
